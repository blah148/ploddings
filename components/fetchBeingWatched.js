...
async function fetchBeingWatched(userId, userIp, limit = null) {
    ...
    let query = supabase
        ...
        .select(`
            page_id,
            content:page_id (id, slug, page_type, name, thumbnail_200x200, featured_img_alt_text)`, { count: 'exact' }) // Assumes a foreign-key relationship named 'content' on 'page_id'
