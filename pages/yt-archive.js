		...
		<>
			...
			<div style={{ margin: '20px', overflowX: 'auto' }}>
				<table style={{ width: '100%', borderCollapse: 'collapse' }}>
					<thead>
						<tr>
							{['song', 'thread', 'playlist_yt_link', 'tablature_link', 'uploaded_at'].map((key) => (
					...
					<tbody>
						{data.map((item, index) => (
							<tr key={index} style={{ backgroundColor: index % 2 ? '#f9f9f9' : '#fff' }}>
								<td>{item.song}</td>
								<td>{item.thread}</td>
								<td><a href={item.playlist_yt_link} target="_blank" rel="noopener noreferrer">{truncateLink(item.playlist_yt_link)}</a></td>
								<td><a href={item.tablature_link} target="_blank" rel="noopener noreferrer">{truncateLink(item.tablature_link)}</a></td>
